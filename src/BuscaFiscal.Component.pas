...
function TBuscaFiscal.GetTributosByGTIN(const AGTIN: string): IBuscaFiscalResponse;
begin
  Result := GetClient.GetTributosByGTIN(AGTIN);
end;

function TBuscaFiscal.SearchTributos(const ADescricao: string; const ALimite: Integer): IBuscaFiscalResponse;
begin
  Result := GetClient.SearchTributos(ADescricao, ALimite);
end;

function TBuscaFiscal.UpdateBatch(const ARequest: IBuscaFiscalBatchRequest): IBuscaFiscalBatchResponse;
begin
